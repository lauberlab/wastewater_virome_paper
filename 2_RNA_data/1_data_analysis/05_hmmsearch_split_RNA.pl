...
# parameters
#
my $indir  = "RNA/assemblies";
my $outdir = "RNA/assemblies_combined";
my $wdir   = "RNA/hmmsearch";
my $parts  = 5000;
...
# combine all contigs
my $infile = "$outdir/scaffolds.fasta";
`rm -rf $infile`;
foreach my $assdir ( glob( "$indir/*" ) ){
	`cat $assdir/spades/scaffolds.fasta >> $infile`;
}
