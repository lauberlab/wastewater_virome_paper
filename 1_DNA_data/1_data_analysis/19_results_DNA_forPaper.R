...
theData <- NULL
if ( DEBUG ){
	theData <- read.delim( "combined_mapping_results_DNA_shuf10k.tsv",  sep="\t", stringsAsFactors=F )
}else{
	theData <- read.delim( "combined_mapping_results_DNA.tsv",          sep="\t", stringsAsFactors=F )
}

# load and join total read count data
theCnt  <- read.delim( "combined_total_read_count_DNA.tsv", sep="\t", header=F, stringsAsFactors=F )
...
theIdent <- NULL
if ( DEBUG ){
	theIdent <- read.delim(  "combined_sequence_identity_results_DNA.tsv",  sep="\t", stringsAsFactors=F )
